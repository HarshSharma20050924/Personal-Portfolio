...
const EliteHero: React.FC<{ data: HeroData; isDark: boolean }> = ({ data, isDark }) => {
  ...
  return (
    <section id="hero" className="relative h-[100svh] w-full flex flex-col justify-center items-center px-4 overflow-hidden pt-24 md:pt-10 bg-white dark:bg-[#050505] transition-colors duration-500">
      ...
      >
        <div className="flex flex-col items-center justify-center leading-[0.85] select-none text-center w-full mix-blend-difference">
            ...
            <motion.h1
               ...
               className="text-[13vw] font-heading font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-black to-gray-500 dark:from-white dark:to-gray-500 select-none z-10"
            >
                {firstName.toUpperCase()}
            </motion.h1>
            
            {/* Last Name - Better Visibility */}
            {lastName && (
                <motion.h1
                    initial={{ y: 30, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    transition={{ duration: 1, delay: 0.1, ease: "easeOut" }}
                    className="text-[13vw] font-heading font-black tracking-tighter text-black/10 dark:text-white/10 select-none relative mt-2 md:-mt-[3vw] z-0"
                    style={{ 
                        WebkitTextStroke: isDark ? '1px rgba(255,255,255,0.6)' : '1px rgba(0,0,0,0.6)'
                    }}
                >
