      ...
      <div className="p-4 md:p-6 bg-white dark:bg-slate-800/50 rounded-lg shadow-md mb-8">
        ...
        <form onSubmit={handleAddNew} className="space-y-6">
          ...
          <div className="border-t border-slate-200 dark:border-slate-700 pt-6">
             <h4 className="text-sm font-bold uppercase text-slate-400 mb-4 tracking-wider">Deep Dive Content</h4>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                
                {/* Core Challenge */}
                <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-900/30 rounded-xl">
                    <div className="flex items-center justify-between">
                        <label className="block text-xs font-bold text-slate-600 dark:text-slate-300">Core Challenge</label>
                    ...
                    <textarea 
                        name="challenge" 
                        value={currentForm.challenge || ''} 
                        onChange={handleFormChange} 
                        placeholder="Describe the main problem..." 
                        rows={4} 
                        className="w-full p-2 text-sm border rounded dark:bg-slate-700 dark:border-slate-600" 
                    />
                    {currentForm.challengeImage && (
                        <div className="h-20 w-full rounded overflow-hidden relative group">
                            <img src={currentForm.challengeImage} className="w-full h-full object-cover" />
                            <div className="absolute inset-0 bg-black/50 hidden group-hover:flex items-center justify-center text-xs text-white cursor-pointer" onClick={() => setForm({...currentForm, challengeImage: ''})}>Remove</div>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-3 p-4 bg-slate-50 dark:bg-slate-900/30 rounded-xl">
                    <div className="flex items-center justify-between">
                        <label className="block text-xs font-bold text-slate-600 dark:text-slate-300">Final Outcome</label>
                        <div className="flex items-center gap-2">
                    ...
                    <textarea 
                        name="outcome" 
                        value={currentForm.outcome || ''} 
                        onChange={handleFormChange} 
                        placeholder="Describe the solution and results..." 
                        rows={4} 
                        className="w-full p-2 text-sm border rounded dark:bg-slate-700 dark:border-slate-600" 
                    />
                    {currentForm.outcomeImage && (
                        <div className="h-20 w-full rounded overflow-hidden relative group">
                            <img src={currentForm.outcomeImage} className="w-full h-full object-cover" />
                            <div className="absolute inset-0 bg-black/50 hidden group-hover:flex items-center justify-center text-xs text-white cursor-pointer" onClick={() => setForm({...currentForm, outcomeImage: ''})}>Remove</div>
                        </div>
                    )}
                </div>
            ...
            <div className="flex items-start space-x-4 mb-4 md:mb-0">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                   {project.videoUrl && <span className="text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded">Video</span>}
                   {project.docUrl && <span className="text-[10px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded">Doc</span>}
                   {(project.challenge || project.outcome) && <span className="text-[10px] bg-purple-100 text-purple-600 px-1.5 py-0.5 rounded">Deep Dive</span>}
                </div>
              </div>
            </div>
